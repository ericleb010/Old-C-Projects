...
main(int argc, char **argv)
{
	...
	// If we have over one argument, something's not right.
	if (argc > 2) {
	...
	// Treat a second argument as a file; allows for manual stdoutput redirection.
	if (argc == 2)
	{
		in = fopen(argv[1], "r");
	}
	...
	{	
		...
		if (fgets(line, LINE_BUFFER, in) == NULL)
		{
		...
		// ...minus the newline and any tab characters.
